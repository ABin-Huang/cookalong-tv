...
| 2:40–2:45 | Terminal: `npm test` → **329 pass**, `npm run verify:browser` → **93/93**, `npm run verify:voice` → **21/21**. | "329 unit tests, 93 checks driven through a real browser at TV resolution, and 21 that drive a real speech recogniser — including two deliberately broken ones: a microphone that never opens, and one that opens, hears you, and gets no words back from its speech service. We didn't just build the demo — we built the thing that proves the demo." |
