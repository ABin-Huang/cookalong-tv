...
/**
 * Integration-style tests for the Alexa skill backend.
 * Constructs real Alexa request envelopes and asserts on the spoken output,
 * which exercises the ASK SDK wiring, intent routing and the shared engines.
 */
...
function launchRequest() {
  return envelope("LaunchRequest", null, {
    type: "LaunchRequest",
...
function intentRequest(sessionAttrs, name, slots = {}) {
  ...
  for (const [key, value] of Object.entries(slots)) {
    slotMap[key] = { name: key, value };
  }
  return envelope("IntentRequest", sessionAttrs, {
    type: "IntentRequest",
    requestId: `req-${name}`,
    timestamp: "2026-01-01T00:00:00Z",
    locale: "en-US",
...
/** Invoke the Lambda-style handler, wrapping its callback API in a Promise. */
...
test("StartCookingIntent starts a known recipe at step 1", async () => {
  const res = await invoke(
    intentRequest(null, "StartCookingIntent", { recipe: "tomato basil pasta" })
  );
  const text = speakText(res);
...
test("StartCookingIntent handles unknown recipe gracefully", async () => {
  const res = await invoke(
    intentRequest(null, "StartCookingIntent", { recipe: "sushi" })
  );
...
test("SetTimerIntent accepts an ISO-8601 AMAZON.DURATION slot", async () => {
  const res = await invoke(
    intentRequest(null, "SetTimerIntent", { duration: "PT5M" })
  );
...
test("SetTimerIntent accepts natural language", async () => {
  const res = await invoke(
    intentRequest(null, "SetTimerIntent", { duration: "90 seconds" })
  );
...
test("SetTimerIntent with invalid duration asks again", async () => {
  const res = await invoke(
    intentRequest(null, "SetTimerIntent", { duration: "sometime" })
  );
...
test("DietFilterIntent lists matching recipes", async () => {
  const res = await invoke(
    intentRequest(null, "DietFilterIntent", { diet: "vegan" })
  );
  const text = speakText(res);
