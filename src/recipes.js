 ...
 * In-memory recipe store with step-by-step instructions, dietary filters,
 * structured ingredients (role-weighted for smart matching) and
 * voice-friendly step formatting. Used by both the Fire TV Web App and
 * the Alexa skill backend.
 *
 * Ingredient shape:
...
/**
 * Return all recipes, optionally filtered by dietary preference.
 * @param {string|null} diet One of DIETS, or null for "any".
 * @returns {Array<object>}
 */
function listRecipes(diet = null) {
  if (!diet) return RECIPES.map(recipe => recipeSummary(recipe));
...
/**
 * Get a full recipe by id or fuzzy name.
 * @param {string} idOrName
 * @returns {object|null}
 */
function getRecipe(idOrName) {
  if (!idOrName) return null;
  const key = String(idOrName).toLowerCase();
  return RECIPES.find(r => r.id === key || r.name.toLowerCase() === key) ||
    RECIPES.find(r => r.name.toLowerCase().includes(key)) || null;
}
 ...
 * @returns {string|null} e.g. "Step 3 of 7: Add 3 crushed garlic cloves..."
 */
function formatStep(recipe, stepNumber) {
  if (!recipe || stepNumber < 1 || stepNumber > recipe.steps.length) return null;
